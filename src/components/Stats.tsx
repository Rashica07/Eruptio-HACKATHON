    ...
    <section className="relative py-32 md:py-56 overflow-hidden bg-background">
      ...
      <div className="absolute inset-0 z-0">
        <video 
          ...
          muted 
          playsInline 
          className="w-full h-full object-cover filter saturate(0) brightness-50 opacity-40"
        >
      ...
      <div className="max-w-[var(--max)] mx-auto px-[var(--gutter)] relative z-10">
        <div className="bg-sage rounded-lg p-12 md:p-24 border border-moss/10 shadow-sm relative overflow-hidden">
          {/* Subtle Graphic */}
          <div className="absolute -bottom-10 -right-10 size-64 bg-moss opacity-10 rounded-full" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12 md:gap-8 relative z-10">
            {STATS.map((stat, idx) => (
              <div key={idx} className="flex flex-col items-center md:items-start text-center md:text-left relative">
                ...
                <div className="font-body text-[10px] md:text-sm text-moss/60 mt-6 tracking-[0.2em] uppercase font-bold">
                  {stat.label}
                </div>
                
                {idx < STATS.length - 1 && (
                  <div className="hidden md:block absolute -right-4 top-1/2 -translate-y-1/2 w-px h-16 bg-moss/10" />
                )}
              </div>
            ))}
          </div>
